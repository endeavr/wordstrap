...
if( ! function_exists( 'wordstrap_add_customizer_section' ) ) {
	function wordstrap_add_customizer_section( $section_id, $section_name, $options, $priority = null ){
		global $_wordstrap_customizer_sections;
		$_wordstrap_customizer_sections[$section_id] = array(
			'id' 		=> $section_id,
			'name' 		=> $section_name,
			'options' 	=> $options,
			'priority'	=> $priority
...
if( ! function_exists( 'wordstrap_registered_customizer_options' ) ) {
	function wordstrap_registered_customizer_options( $sections ){
...
if( ! function_exists( 'wordstrap_customizer_init' ) ) {
	function wordstrap_customizer_init( $wp_customize ){
		...
		if( $_wordstrap_customizer_sections ) {
			foreach( $_wordstrap_customizer_sections as $section ){
					...
					foreach( $section['options'] as $option ) {
						...
						if( $option['type'] == 'logo' ){
							...
							$wp_customize->add_setting( $option_name.'['.$option['id'].'][type]', array(
								...
								'type'       	=> 'option',
								'capability' 	=> wordstrap_admin_module_cap( 'options' ),
								'transport'		=> $transport
							) );
							$wp_customize->add_control( $option['id'].'_type', array(
								'priority'		=> 1,
								'settings'		=> $option_name.'['.$option['id'].'][type]',
								'label'   		=> $option['name'].' '.__( 'Type', WS_GETTEXT_DOMAIN ),
								'section'    	=> $section['id'],
								'type'       	=> 'select',
								'choices'    	=> array(
							...
							$wp_customize->add_control( $option['id'].'_custom', array(
								...
								'label'      	=> __( 'Custom Title', WS_GETTEXT_DOMAIN ),
								'section'    	=> $section['id']
							) );
							...
							$wp_customize->add_control( $option['id'].'_custom_tagline', array(
								...
								'label'      	=> __( 'Custom Tagline', WS_GETTEXT_DOMAIN ),
								'section'    	=> $section['id']
							) );
							...
							$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, $option['id'].'_image', array(
								...
								'label'   => $option['name'].' '.__( 'Image', WS_GETTEXT_DOMAIN ),
								'section' => $section['id'],
							) ) );
							...
							foreach( $option['atts'] as $attribute ){
								...
								switch( $attribute ){
									case 'size' :
										...
										break;
									case 'face' :
										$wp_customize->add_control( new WP_Customize_WordStrap_Font_Face( $wp_customize, $option['id'].'_'.$attribute, array(
											'priority'		=> $font_counter,
											'settings'		=> $option_name.'['.$option['id'].']['.$attribute.']',
											'label'   		=> $option['name'].' '.ucfirst($attribute),
											'section' 		=> $section['id'],
										...
										$wp_customize->add_control( new WP_Customize_WordStrap_Google_Font( $wp_customize, $option['id'].'_'.$attribute.'_google', array(
											'priority'		=> $font_counter,
											'settings'		=> $option_name.'['.$option['id'].'][google]',
											'label'   		=> __( 'Google Font Name', WS_GETTEXT_DOMAIN ),
											'section' 		=> $section['id']
										) ) );
										$font_counter++;
										break;
									case 'style' :
										// Currently not supported in WordStrap options 
										// $counter++;
										break;
									case 'color' :
								...
								$wp_customize->add_control( new WP_Customize_WordStrap_Divider( $wp_customize, $option['id'].'_divider', array(
									'priority'		=> $font_counter,
							...
							switch( $option['type'] ) {
								...
								// Textarea
								case 'textarea' :
									$wp_customize->add_control( new WP_Customize_WordStrap_Textarea( $wp_customize, $option['id'], array(
										'priority'		=> $priority,
										'settings'		=> $option_name.'['.$option['id'].']',
										'label'   		=> $option['name'],
										'section' 		=> $section['id']
		...
		$remove_sections = apply_filters( 'wordstrap_customizer_remove_sections', array( 'title_tagline' ) );
		if( is_array( $remove_sections ) && $remove_sections )
			foreach( $remove_sections as $section )
				$wp_customize->remove_section( $section );
		
		// Modify sections
		$modify_sections = apply_filters( 'wordstrap_customizer_modify_sections', array( 'static_front_page' ) );
				...
				switch( $section ) {
					case 'static_front_page' :
						...
						$wp_customize->add_section( 'static_front_page', array(
							'title'          => __( 'Homepage', WS_GETTEXT_DOMAIN ),
							'priority'       => 120,
						...
						$wp_customize->add_control( 'homepage_content', array(
							...
							'label'			=> __( 'Homepage Content', WS_GETTEXT_DOMAIN ),
							'section'		=> 'static_front_page',
							'type'			=> 'radio',
							'choices'		=> array(
								'posts'			=> __( 'WordPress Default', WS_GETTEXT_DOMAIN ),
						...
						$wp_customize->add_control( 'homepage_custom_layout', array(
							...
							'label'			=> __( 'Homepage Custom Layout', WS_GETTEXT_DOMAIN ),
							'section'		=> 'static_front_page',
							'type'			=> 'select',
...
if( ! function_exists( 'wordstrap_customizer_styles' ) ) {
	function wordstrap_customizer_styles(){
		wp_register_style( 'wordstrap_customizer', get_template_directory_uri().'/framework/admin/assets/css/customizer.css', false, '1.0' );
		wp_enqueue_style( 'wordstrap_customizer' );
	}
}
...
if( ! function_exists( 'wordstrap_customizer_scripts' ) ) {
	function wordstrap_customizer_scripts(){
		wp_register_script( 'wordstrap_customizer', get_template_directory_uri().'/framework/admin/assets/js/customizer.min.js', array('jquery'), '1.0');
	    wp_enqueue_script( 'wordstrap_customizer' );
	    wp_localize_script( 'wordstrap_customizer', 'wordstrap', wordstrap_get_admin_locals( 'customizer_js' ) );	
	}
}
...
if( class_exists( 'WP_Customize_Control' ) ) {
	...
	class WP_Customize_WordStrap_Textarea extends WP_Customize_Control {
		
		public $type = 'textarea';
		public $statuses;
	
		public function __construct( $manager, $id, $args = array() ) {
			$this->statuses = array( '' => __('Default', WS_GETTEXT_DOMAIN ) );
	...
	class WP_Customize_WordStrap_Font_Face extends WP_Customize_Control {
		...
		public function render_content() {		
			if ( empty( $this->choices ) )
				return;
			?>
			<label class="wordstrap-font-face">
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
	...
	class WP_Customize_WordStrap_Google_Font extends WP_Customize_Control {
		...
		public function render_content() {
			?>		
			<label class="wordstrap-google-font">
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
				<input type="text" value="<?php echo esc_attr( $this->value() ); ?>" <?php $this->link(); ?> />
				<p><?php _e( 'Example', WS_GETTEXT_DOMAIN ); ?>: Pontano Sans</p>
				<p><a href="http://www.google.com/webfonts" target="_blank"><?php _e( 'Browse Google Fonts', WS_GETTEXT_DOMAIN ); ?></a></p>
			</label>
			<?php
		}
	...
	class WP_Customize_WordStrap_Divider extends WP_Customize_Control {
		...
		public function render_content() {
			?>		
			<div class="wordstrap-divider"></div>
			<?php
		}
...
if( ! function_exists( 'wordstrap_customizer_preview_logo' ) ) {
	function wordstrap_customizer_preview_logo(){
		
		// Global option name
		$option_name = wordstrap_get_option_name();
		
		// Setup for logo
		$logo_options = wordstrap_get_option('logo');
		$logo_atts = array(
			'type' 				=> '',
			'site_url'			=> home_url(),
		...
		wp.customize('<?php echo $option_name; ?>[logo][type]',function( value ) {
			value.bind(function(value) {
				...
				if( value == 'title' )
				{
					...
					$('#branding .header_logo').html('<h1 class="ws-text-logo"><a href="'+Logo.site_url+'" title="'+Logo.title+'">'+Logo.title+'</a></h1>');
				}
				else if( value == 'title_tagline' )
				{
					...
					$('#branding .header_logo').html('<h1 class="ws-text-logo"><a href="'+Logo.site_url+'" title="'+Logo.title+'">'+Logo.title+'</a></h1><span class="tagline">'+Logo.tagline+'</span>');
				}
				else if( value == 'custom' )
				{
					var html = '<h1 class="ws-text-logo"><a href="'+Logo.site_url+'" title="'+Logo.custom+'">'+Logo.custom+'</a></h1>';
				...
				else if( value == 'image' )
				{
					...
					if(Logo.image)
					{
						html = '<a href="'+Logo.site_url+'" title="'+Logo.title+'" class="ws-image-logo"><img src="'+Logo.image+'" alt="'+Logo.title+'" /></a>';
					}
					else
					{
...
if( ! function_exists( 'wordstrap_customizer_preview_font_prep' ) ) {
	function wordstrap_customizer_preview_font_prep(){
		
		// Global option name
		$option_name = wordstrap_get_option_name();
		
		// Setup font stacks
		$font_stacks = wordstrap_font_stacks();
		...
		foreach( $google_fonts as $key => $value ){
			$font = wordstrap_get_option('typography_'.$key);
...
if( ! function_exists( 'wordstrap_customizer_preview_primary_font' ) ) {
	function wordstrap_customizer_preview_primary_font(){
...
if( ! function_exists( 'wordstrap_customizer_preview_header_font' ) ) {
	function wordstrap_customizer_preview_header_font(){
...
if( ! function_exists( 'wordstrap_customizer_preview_styles' ) ) {
	function wordstrap_customizer_preview_styles(){

...
if( ! function_exists( 'wordstrap_add_slider' ) ) {
	function wordstrap_add_slider( $slider_id, $slider_name, $slide_types, $media_positions, $slide_elements, $options, $function_to_display ) {
		...
		foreach( $media_positions as $position ) {
			switch( $position ) {
				case 'full' :
					$formatted_media_positions['full'] = __( 'Full-Size', WS_GETTEXT_DOMAIN );
					break;
				case 'align-left' :
					$formatted_media_positions['align-left'] = __( 'Aligned Left', WS_GETTEXT_DOMAIN );
					break;
				case 'align-right' :
					$formatted_media_positions['align-right'] = __( 'Aligned Right', WS_GETTEXT_DOMAIN );
					break;
			}	
		}
		...
		// Add in slider
		$_wordstrap_user_sliders[$slider_id] = array(
			'name' 		=> $slider_name,
			'id'		=> $slider_id,
		...
		add_action( 'wordstrap_'.$slider_id.'_slider', $function_to_display, 10, 3 );
	}
}	
...
if( ! function_exists( 'wordstrap_standard_slider_js' ) ) {
	function wordstrap_standard_slider_js( $id, $options ) {
		?>
		<script>
		jQuery(document).ready(function($) {
			$(window).load(function() {
				...
				$('#ws-slider-<?php echo $id; ?> .flexslider').flexslider({
					...
					start: function(slider) {
        				<?php if( $options['pause_play'] && $options['timeout'] != '0' ) : ?>
		    				$('#ws-slider-<?php echo $id; ?> .flex-direction-nav li:first-child').after('<li><a class="flex-pause" href="#">Pause</a></li><li><a class="flex-play" href="#" style="display:none">Play</a></li>');
		    				$('#ws-slider-<?php echo $id; ?> .flex-pause').click(function(){
								slider.pause();
								$(this).hide();
								$('#ws-slider-<?php echo $id; ?> .flex-play').show();
								return false;
							});
							$('#ws-slider-<?php echo $id; ?> .flex-play').click(function(){
								// slider.resume(); currently has a bug with FlexSlider 2.0, so will do the next line instead.
								$('#ws-slider-<?php echo $id; ?> .flexslider').flexslider('play');
								$(this).hide();
								$('#ws-slider-<?php echo $id; ?> .flex-pause').show();
								return false;
							});
							$('#ws-slider-<?php echo $id; ?> .flex-control-nav li, #ws-slider-<?php echo $id; ?> .flex-direction-nav li').click(function(){
								$('#ws-slider-<?php echo $id; ?> .flex-pause').hide();
								$('#ws-slider-<?php echo $id; ?> .flex-play').show();
							});
						<?php endif; ?>
        				$('#ws-slider-<?php echo $id; ?> .image-link').click(function(){
        					$('#ws-slider-<?php echo $id; ?> .flex-pause').hide();
        					$('#ws-slider-<?php echo $id; ?> .flex-play').show();
        					slider.pause();
        				});
        			}
				}).parent().find('.ws-loader').fadeOut();
				...
				<?php if( isset( $options['pause_on_hover'] ) ) : ?>
					...
					$('#ws-slider-<?php echo $id; ?>').hover(
						function() {
							$('#ws-slider-<?php echo $id; ?> .flex-pause').hide();
							$('#ws-slider-<?php echo $id; ?> .flex-play').show();
							$('#ws-slider-<?php echo $id; ?> .flexslider').flexslider('pause');
						}, 
						function() {
							<?php if( $options['pause_on_hover'] == 'pause_on_off' ) : ?>
							$('#ws-slider-<?php echo $id; ?> .flex-play').hide();
							$('#ws-slider-<?php echo $id; ?> .flex-pause').show();
							$('#ws-slider-<?php echo $id; ?> .flexslider').flexslider('play');
							<?php endif; ?>
						}
...
if( ! function_exists( 'wordstrap_carrousel_slider_js' ) ) {
	function wordstrap_carrousel_slider_js( $id, $options ) {
		?>
		<script>
		jQuery(document).ready(function($) {
			$(window).load(function() {
				$('#ws-slider-<?php echo $id; ?> .ws-loader').fadeOut('fast');
				$('#ws-slider-<?php echo $id; ?> .slider-inner').fadeIn('fast');
				$('#ws-slider-<?php echo $id; ?> .carrousel-slider').roundabout({
					// minOpacity: '1',
					responsive: true,
					<?php if( $options['nav_arrows'] ) : ?>
					btnNext: '#ws-slider-<?php echo $id; ?> .next',
         			btnPrev: '#ws-slider-<?php echo $id; ?> .prev'
         			<?php endif; ?>
				});
			});
		});
...
/**
 * Standard Slider - default action for wordstrap_standard_slider
 *
 * @since 2.0.0
 *
...
if( ! function_exists( 'wordstrap_standard_slider_default' ) ) {
	function wordstrap_standard_slider_default( $slider, $settings, $slides ) {
		
		// Configure additional CSS classes
		$classes = wordstrap_get_classes( 'slider_standard', true );
		...
		// Start output
		wordstrap_standard_slider_js( $slider, $settings );
		?>
		<div id="ws-slider-<?php echo $slider; ?>" class="slider-wrapper standard-slider-wrapper<?php if($hide) echo ' slider_has_mobile_fallback';?>">
									...
									<?php foreach( $slides as $slide ) : ?>
										...
										if( ! isset( $slide['custom'] ) ) {
											...
											if( $slide['slide_type'] == 'video' ) {	
												...
												// Set error message
												if( ! $video )
													$video = '<p>'.wordstrap_get_local( 'no_video' ).'</p>';
											}
											// Elements
											$elements = array();
																		...
																		<div class="slide-description">
																			<span>
																				...
																				<?php if( in_array( 'button', $elements ) && $slide['elements']['button']['text'] ) : ?>
																					<p class="slide-description-button"><?php echo wordstrap_button( stripslashes( $slide['elements']['button']['text'] ), $slide['elements']['button']['url'], 'default', $slide['elements']['button']['target'], 'medium' ); ?></p>
																				<?php endif; ?>
																			</span>
																		</div><!-- .description (end) -->
																	<?php endif; ?>
																...
																<?php if( $slide['slide_type'] == 'image' ) : ?>
																	<?php if( in_array( 'image_link', $elements ) && $slide['elements']['image_link']['url'] ) : ?>
																		<?php if( $slide['elements']['image_link']['target'] == 'lightbox' ) : ?>
																			<a href="<?php echo $slide['elements']['image_link']['url']; ?>" class="image-link enlarge" rel="wordstrap_lightbox" title=""><span>Image Link</span></a>
																		<?php else : ?>
		...
		if( isset( $settings['mobile_fallback'] ) )
			if( $settings['mobile_fallback'] == 'full_list' || $settings['mobile_fallback'] == 'first_slide' )
				wordstrap_slider_fallback( $slider, $slides, $settings['mobile_fallback'] );
	}
}

/**
 * Carrousel Slider - default action for wordstrap_carrousel_slider
 *
 * @since 2.0.0
 *
 * @param var $slider ID of current slider
...
if( ! function_exists( 'wordstrap_carrousel_slider_default' ) ) {
	function wordstrap_carrousel_slider_default( $slider, $settings, $slides ) {
		wordstrap_carrousel_slider_js( $slider, $settings );
		$classes = wordstrap_get_classes( 'slider_carrousel', true );
		...
		?>
		<div id="ws-slider-<?php echo $slider; ?>" class="slider-wrapper carrousel-slider-wrapper<?php echo $classes; ?>">
			<div class="ws-loader"></div>
...
if( ! function_exists( 'wordstrap_slider_fallback' ) ) {
	function wordstrap_slider_fallback( $slider, $slides, $fallback ) {
		...
		foreach( $slides as $slide ) {
			if( ! isset( $slide['custom'] ) ) {
				...
				if( $slide['slide_type'] == 'image' ) {
					...
					$image_size = apply_filters( 'wordstrap_slider_fallback_img_size', $image_size, $fallback, $slide['position'] ); // Apply optional filter and pass in fallback type & image position
					$image_url = null;
					$image_title = null;
					if( isset( $slide['image'][$image_size] ) && $slide['image'][$image_size] )
						$image_url = $slide['image'][$image_size]; // We do a strict check here so no errors will be thrown with old versions of the framework.
			...
			echo '<div class="slider-fallback-slide-body">';
				if( isset( $slide['custom'] ) ) {
					...
					if( $slide['slide_type'] == 'image' ) {
						if( in_array( 'image_link', $elements ) ) {
							if( $slide['elements']['image_link']['target'] == 'lightbox' )
								echo '<a href="'.$slide['elements']['image_link']['url'].'" class="image-link enlarge" rel="wordstrap_lightbox">';
							else
					...
					if( in_array( 'button', $elements ) && isset( $slide['elements']['button']['text'] ) && $slide['elements']['button']['text'] )
						echo '<p class="slide-description-button">'.wordstrap_button( stripslashes( $slide['elements']['button']['text'] ), $slide['elements']['button']['url'], 'default', $slide['elements']['button']['target'], 'medium' ).'</p>';
				}

...
if( ! function_exists( 'wordstrap_frontend_init' ) ) {
	function wordstrap_frontend_init() {

		global $_wordstrap_theme_settings;
		global $_wordstrap_user_stylesheets;
		global $_wordstrap_config;
		global $post;
		...
		$_wordstrap_theme_settings = get_option( $option_name );
		...
		$fake_conditional = wordstrap_get_fake_conditional();
		...
		if( is_page_template( 'template_builder.php' ) ) {
			$layout_id = get_post_meta( $post->ID, '_ws_custom_layout', true );
			if( $layout_id ) {
				$builder = $layout_id;
				$layout_post_id = wordstrap_post_id_by_name( $layout_id, 'ws_layout' );
		...
		if( is_home() ) {
			$homepage_content = wordstrap_get_option( 'homepage_content', null, 'posts' );
			if( $homepage_content == 'custom_layout' ) {
				$layout_id = wordstrap_get_option( 'homepage_custom_layout' );
				if( $layout_id ) {
					$builder = $layout_id;
					$layout_post_id = wordstrap_post_id_by_name( $layout_id, 'ws_layout' );
					$layout_settings = get_post_meta( $layout_post_id, 'settings', true );
		...
		if( $builder ) {
			$layout_post_id = wordstrap_post_id_by_name( $builder, 'ws_layout' );
			$elements = get_post_meta( $layout_post_id, 'elements', true );
			$featured = wordstrap_featured_builder_classes( $elements, 'featured' );
			$featured_below = wordstrap_featured_builder_classes( $elements, 'featured_below' );	
		}
		if( is_home() ) {
			$homepage_content = wordstrap_get_option( 'homepage_content', null, 'posts' );
			if( $homepage_content != 'custom_layout' ) {
				if( wordstrap_get_option( 'blog_featured' ) || wordstrap_supports( 'featured', 'blog' ) )
					$featured[] = 'has_blog_featured';
				if( wordstrap_supports( 'featured_below', 'blog' ) )
		...
		if( is_page_template( 'template_grid.php' ) ) {
			if( wordstrap_supports( 'featured', 'grid' ) )
				$featured[] = 'has_grid_featured';
			if( wordstrap_supports( 'featured_below', 'grid' ) )
				$featured_below[] = 'has_grid_featured_below';
		}
		if( is_archive() || is_search() ){
			if( wordstrap_supports( 'featured', 'archive' ) )
				$featured[] = 'has_archive_featured';
			if( wordstrap_supports( 'featured_below', 'archive' ) )
				$featured_below[] = 'has_archive_featured_below';
		}
		if( is_page() && ! is_page_template( 'template_builder.php' ) ) {
			if( wordstrap_supports( 'featured', 'page' ) )
				$featured[] = 'has_page_featured';
			if( wordstrap_supports( 'featured_below', 'page' ) )
				$featured_below[] = 'has_page_featured_below';
		}
		if( is_single() ) {
			if( wordstrap_supports( 'featured', 'single' ) )
				$featured[] = 'has_single_featured';
			if( wordstrap_supports( 'featured_below', 'single' ) )
		...
		if( ! $sidebar_layout || 'default' == $sidebar_layout ) {
			$sidebar_layout = wordstrap_get_option( 'sidebar_layout', null, apply_filters( 'wordstrap_default_sidebar_layout', 'sidebar_right' ) );
		}
		$sidebar_layout = apply_filters( 'wordstrap_sidebar_layout', $sidebar_layout );
    	...
    	$_wordstrap_config = apply_filters( 'wordstrap_frontend_config', $config );
	}
}
...
if( ! function_exists( 'wordstrap_featured_builder_classes' ) ) {
	function wordstrap_featured_builder_classes( $elements, $area ) {
		$classes = array();
...
if( ! function_exists( 'wordstrap_body_class' ) ) {
	function wordstrap_body_class( $classes ) {
		
		// Featured Area
		if( wordstrap_config( 'featured' ) )
			$classes[] = 'show-featured-area';
		else
			$classes[] = 'hide-featured-area';
			
		// Featured Area (below)
		if( wordstrap_config( 'featured_below' ) )
			$classes[] = 'show-featured-area-below';
		else
			$classes[] = 'hide-featured-area-above';

		// Custom Layout
		$custom_layout = wordstrap_config( 'builder' );
		if( $custom_layout ) {
		...
		// Sidebar Layout
		$classes[] = 'sidebar-layout-'.wordstrap_config( 'sidebar_layout' );
...
if( ! function_exists( 'wordstrap_get_fake_conditional' ) ) {
	function wordstrap_get_fake_conditional() {
		$fake_conditional = '';
		if( is_home() )
 ...
 * @since 2.0.0
 * 
 * @param $key string $key to retrieve from $_wordstrap_config
 * @return $value mixed value from $_wordstrap_config
 */

function wordstrap_config( $key, $seconday = null ) {
	global $_wordstrap_config;
	$value = null;
	if( $seconday ) {
		if( isset( $_wordstrap_config[$key][$seconday] ) )
			$value = $_wordstrap_config[$key][$seconday];	
	} else {	
		if( isset( $_wordstrap_config[$key] ) )
			$value = $_wordstrap_config[$key];
	}
	return $value;
}
...
function wordstrap_get_assigned_id( $assignments ) {
...
if( ! function_exists( 'wordstrap_include_scripts' ) ) {
	function wordstrap_include_scripts() {
		// Register scripts
		// wp_register_script( 'prettyPhoto', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/plugins/prettyphoto/js/jquery.prettyPhoto.js', array('jquery'), '3.1.3' );
		wp_register_script( 'prettyPhoto', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/js/prettyphoto.js', array('jquery'), '3.1.3' ); // Modified version of prettyPhoto by Jason Loftis
		wp_register_script( 'superfish', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/js/superfish.js', array('jquery'), '1.4.8' );
		// wp_register_script( 'flexslider', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/js/flexslider.js', array('jquery'), '1.8' );
		wp_register_script( 'flexslider', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/js/flexslider-2.js', array('jquery'), '2.0' );
		wp_register_script( 'roundabout', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/js/roundabout.js', array('jquery'), '1.1' );
		wp_register_script( 'wordstrap', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/js/wordstrap.js', array('jquery'), '1.0' );
		wp_register_script( 'ios-orientationchange-fix', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/js/ios-orientationchange-fix.js' );
		// Enqueue 'em
...
if( ! function_exists( 'wordstrap_include_styles' ) ) {
	function wordstrap_include_styles() {
		// Level 1 user styles
		wordstrap_user_stylesheets( 1 );
		// Register framework styles
		wp_register_style( 'prettyPhoto', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/plugins/prettyphoto/css/prettyPhoto.css' );
		wp_register_style( 'wordstrap_plugins', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/css/plugins.css' );
		wp_register_style( 'wordstrap', WS_FRAMEWORK_DIRECTORY . '/frontend/assets/css/wordstrap.css' );
		// Enqueue framework styles
		wp_enqueue_style( 'prettyPhoto' );
		wp_enqueue_style( 'wordstrap_plugins' );
		wp_enqueue_style( 'wordstrap' );
		// Level 2 user styles
		wordstrap_user_stylesheets( 2 );
	}
}
...
if( ! function_exists( 'wordstrap_include_google_fonts' ) ) {
	function wordstrap_include_google_fonts() {
		$fonts = func_get_args();
		if( ! empty( $fonts ) ) {
			...
			foreach( $fonts as $font ) {
				if( $font['face'] == 'google' && $font['google'] ) {
					$name = wordstrap_remove_trailing_char( $font['google'] ); 
...
if( ! function_exists( 'wordstrap_font_stacks' ) ) {
	function wordstrap_font_stacks() {
		$stacks = array(
		...
		$stacks = apply_filters( 'wordstrap_font_stacks', $stacks );
		return $stacks;
	}
}
...
if( ! function_exists( 'wordstrap_sidebar_layout_class' ) ) {
	function wordstrap_sidebar_layout_class() {
		global $_wordstrap_config;
		echo $_wordstrap_config['sidebar_layout'];
	}
}
...
if( ! function_exists( 'wordstrap_get_part' ) ) {
	function wordstrap_get_part( $type ) {
		$part = null;
		...
		$parts = apply_filters( 'wordstrap_template_parts', $parts );
...
if( ! function_exists( 'wordstrap_user_stylesheets' ) ) {
	function wordstrap_user_stylesheets( $level ) {
		global $_wordstrap_user_stylesheets;
		// Add styles
		if( $level == 4 ) {
			// Manually insert level 4 stylesheet
			if( $_wordstrap_user_stylesheets[4] )
				foreach( $_wordstrap_user_stylesheets[4] as $stylesheet )
			...
			if( $_wordstrap_user_stylesheets[$level] )
				foreach( $_wordstrap_user_stylesheets[$level] as $stylesheet )
...
if( ! function_exists( 'wordstrap_closing_styles' ) ) {
	function wordstrap_closing_styles() {
		// Show user stylesheets after 
		// ALL styles if any exist.
		wordstrap_user_stylesheets( 4 );
	}
}
 ...
 * added as a filter to wordstrap_sidebar_layout, which gets 
 * applied in wordstrap_frontend_init.
 *
 * @since 2.1.0
 */

if( ! function_exists( 'wordstrap_wpmultisite_signup_sidebar_layout' ) ) {
	function wordstrap_wpmultisite_signup_sidebar_layout( $sidebar_layout ) {
		global $pagenow;
...
if( ! function_exists( 'wordstrap_homepage_posts_per_page' ) ) {
	function wordstrap_homepage_posts_per_page( $query ){

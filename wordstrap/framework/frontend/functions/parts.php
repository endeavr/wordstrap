...
if( ! function_exists( 'wordstrap_contact_bar' ) ) {
	function wordstrap_contact_bar( $buttons = array(), $style = null ) {
		// Set up buttons
		if( ! $buttons )
			$buttons = wordstrap_get_option( 'social_media' );
		// If buttons haven't been sanitized return nothing
		if( is_array( $buttons ) && isset( $buttons['includes'] ) )
			return null;
		// Set up style
		if( ! $style )
			$style = wordstrap_get_option( 'social_media_style', null, 'grey' );
		// Start output
		$output = null;
		if( is_array( $buttons ) && ! empty ( $buttons ) ) {
			$output = '<div class="wordstrap-contact-bar">';
			...
			$output .= '</div><!-- .wordstrap-contact-bar (end) -->';
		}
		return $output;
	}
}
...
if( ! function_exists( 'wordstrap_button' ) ) {
	function wordstrap_button( $text, $url, $color = 'default', $target = '_self', $size = 'small', $classes = null, $title = null ) {
		if( $target == 'lightbox' ) {
			$lightbox = ' rel="wordstrap_lightbox"';
			$target = null;
		} else {
			$lightbox = null;
		...
		if( ! $title )
			$title = $text;
		$output = '<a href="'.$url.'" title="'.$title.'" class="ws-button ws-button-'.$size.' '.$color.' '.$classes.'"'.$target.$lightbox.'>';
...
if( ! function_exists( 'wordstrap_archive_title' ) ) {
	function wordstrap_archive_title() {
		global $post;
		global $posts;
	    if( $posts ) $post = $posts[0]; // Hack. Set $post so that the_date() works.
	    if ( is_search() ) {
			// Search Results
			echo wordstrap_get_local('crumb_search').' "'.get_search_query().'"';
	    } else if ( is_category() ) {
	    	// If this is a category archive 
	    	// echo wordstrap_get_local( 'category' ).': ';
	    	single_cat_title();
	    } else if( is_tag() ) {
	    	// If this is a tag archive 
	    	echo wordstrap_get_local('crumb_tag').' "'.single_tag_title('', false).'"';
	    } else if ( is_day() ) {
	    	// If this is a daily archive 
	    	echo wordstrap_get_local( 'archive' ).': ';
	    	the_time('F jS, Y');
	    } else if ( is_month()) {
			...
			echo wordstrap_get_local('crumb_author').' '.$userdata->display_name;
	    }
	}
}
...
if( ! function_exists( 'wordstrap_pagination' ) ) {
	function wordstrap_pagination( $pages = '', $range = 2 ) {  
		global $paged;
		global $_wordstrap_paged;
		$showitems = ($range * 2)+1;  	     
		if( $_wordstrap_paged ) $paged = $_wordstrap_paged; // Static frontpage compatibility
...
if( ! function_exists( 'wordstrap_get_breadcrumbs' ) ) {
	function wordstrap_get_breadcrumbs() {
		global $post;
		// Filterable attributes
		$atts = array(
			'delimiter' => '&raquo;',
			'home' => wordstrap_get_local('home'),
			'home_link' => home_url(),
			'before' => '<span class="current">',
			'after' => '</span>'
		);
		$atts = apply_filters( 'wordstrap_breadcrumb_atts', $atts );
		// Start output
		...
		if ( is_category() ) {
			...
			$output .= $atts['before'].wordstrap_get_local('crumb_search').' "'.get_search_query().'"'.$atts['after'];
			...
			$output .= $atts['before'].wordstrap_get_local('crumb_tag').' "'.single_tag_title('', false).'"'.$atts['after'];
		} else if ( is_author() ) {
			global $author;
			$userdata = get_userdata($author);
			$output .= $atts['before'].wordstrap_get_local('crumb_author').' '.$userdata->display_name.$atts['after'];
		} else if ( is_404() ) {
		  $output .= $atts['before'].wordstrap_get_local('crumb_404').$atts['after'];
		}
		if ( get_query_var('paged') ) {
			$output .= ' ('.wordstrap_get_local('page').' '.get_query_var('paged').')';
		}
...
if( ! function_exists( 'wordstrap_get_twitter' ) ) {
	function wordstrap_get_twitter( $count, $username, $time = 'yes', $exclude_replies = 'yes' ) {		
		...
		$rss = fetch_feed( wordstrap_get_twitter_rss_url( $username ) );
		...
		if ( ! is_wp_error( $rss ) ) {
				...
				foreach ( $rss_items as $item ) {
					...
					if( $exclude_replies == 'no' || ( $exclude_replies == 'yes' && $text[0] != "@") ) {
					    ...
					    $tweets[] = array(
					    	...
					    	'text' => apply_filters( 'wordstrap_tweet_filter', $text, $username ),
			...
			if( $output )
				...
				$output = '<ul class="tweets"><li>'.__( 'No public Tweets found', WS_GETTEXT_DOMAIN ).'</li></ul>';
			
		} else {
			// Received error with fetch_feed()
			$output = '<ul class="tweets"><li>'.__( 'Could not fetch Twitter RSS feed.', WS_GETTEXT_DOMAIN ).'</li></ul>';
		}
		return $output;
	}
}
...
if( ! function_exists( 'wordstrap_twitter' ) ) {
	function wordstrap_twitter( $count, $username, $time = 'yes', $exclude_replies = 'yes' ) {	
		echo wordstrap_get_twitter( $count, $username, $time, $exclude_replies );
	}
}
...
if( ! function_exists( 'wordstrap_nav_menu_select' ) ) {
	function wordstrap_nav_menu_select( $location ) {
		$select_menu = '';
		...
		if( isset( $locations[$location] ) ) {
			...
			if( is_object( $menu ) ) {
				...
				if( ! empty( $menu_items ) ) {
					...
					$select_menu .= '<select class="ws-jump-menu">';
					$select_menu .= '<option value="">'.wordstrap_get_local('navigation').'</option>';
...
if( ! function_exists( 'wordstrap_get_simple_contact' ) ) {
	function wordstrap_get_simple_contact( $args ) {
		// Setup icon links
		$icons = array();
		...
		if( isset( $args['contact'] ) && $args['contact'] )
			$module .= '<li class="contact"><a href="'.$args['contact'].'">'.wordstrap_get_local( 'contact_us' ).'</a></li>';
		// Skype
...
if( ! function_exists( 'wordstrap_simple_contact' ) ) {
	function wordstrap_simple_contact( $args ) {
		echo wordstrap_get_simple_contact( $args );
	}
}
...
if( ! function_exists( 'wordstrap_get_mini_post_list' ) ) {
	function wordstrap_get_mini_post_list( $query = '', $thumb = 'smaller', $meta = true ) {
		global $post;
		$output = '';
		// CSS classes
		$classes = '';
		...
		// Start output
		if( $posts ) {
			$output  = '<div class="wordstrap-mini-post-list">';
			...
			foreach( $posts as $post ) {
				...
				if( $thumb ) {
					$image = wordstrap_get_post_thumbnail( 'primary', 'square_'.$thumb, true, false );
					...
					if( ! $image ) {
						$default_img_directory = apply_filters( 'wordstrap_thumbnail_directory', get_template_directory_uri() . '/framework/frontend/assets/images/thumbs/' );
			...
			wp_reset_postdata();
			$output .= '<ul>';
			$output .= '</div><!-- .wordstrap-mini-post-list (end) -->';
		} else {
			$output = wordstrap_get_local( 'archive_no_posts' );
		}
		return $output;
	}
}
...
if( ! function_exists( 'wordstrap_mini_post_list' ) ) {
	function wordstrap_mini_post_list( $options ) {
		echo wordstrap_get_mini_post_list( $options );
	}
}
...
if( ! function_exists( 'wordstrap_get_mini_post_grid' ) ) {
	function wordstrap_get_mini_post_grid( $query = '', $align = 'left', $thumb = 'smaller', $gallery = '' ) {
		global $post;
		$output = '';
		// CSS classes
		$classes = $thumb.'-thumbs';
		...
		// Start output
		if( $posts ) {
			$output  = '<div class="wordstrap-mini-post-grid">';
			$output .= '<ul class="'.$classes.'">';
			foreach( $posts as $post ) {
				setup_postdata( $post );
				$output .= '<li>';
				if( $gallery ) {
					...
					$output .= '<a href="'.$image[0].'" title="" class="image" rel="wordstrap_lightbox[gallery_'.$gallery.']">';
					$output .= '<img src="'.$thumbnail[0].'" alt="'.$post->post_title.'" />';
					$output .= apply_filters( 'wordstrap_image_overlay', '<span class="image-overlay"><span class="image-overlay-bg"></span><span class="image-overlay-icon"></span></span>' );
					$output .= '</a>';
...
if( ! function_exists( 'wordstrap_mini_post_grid' ) ) {
	function wordstrap_mini_post_grid( $query = '', $align = 'left', $thumb = 'smaller', $gallery = '' ) {
		echo wordstrap_get_mini_post_grid( $query, $align, $thumb, $gallery );
	}
}
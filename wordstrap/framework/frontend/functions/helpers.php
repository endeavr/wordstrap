...
if( ! function_exists( 'wordstrap_get_post_thumbnail' ) ) {
	function wordstrap_get_post_thumbnail( $location = 'primary', $size = '', $link = true, $allow_filters = true, $gallery = 'gallery' ) {
		global $post;
		$attachment_id = get_post_thumbnail_id( $post->ID );
		$conditionals = apply_filters( 'thumbnail_conditionals', array( 'home', 'template_list.php', 'single', 'search', 'archive' ) );
		$fake_conditional = wordstrap_config( 'fake_conditional' );
		$sidebar_layout = wordstrap_config( 'sidebar_layout' );
		$link_target = null;
		$end_link = null;
		$output = null;
		$classes = null;
		...
		if( ! $size ) {
			...
			if( in_array( $fake_conditional, $conditionals ) ) {
				...
				if( $fake_conditional == 'home' || $fake_conditional == 'template_list.php' )	
					$thumb_size_option = wordstrap_get_option( 'blog_thumbs', null, 'small' );
				else if( $fake_conditional == 'search' || $fake_conditional == 'archive' )
					$thumb_size_option = wordstrap_get_option( 'archive_thumbs', null, 'small' );
				else if( $fake_conditional == 'single' )
					$thumb_size_option = wordstrap_get_option( 'single_thumbs', null, 'small' );
				// Single post page override
				if( $fake_conditional == 'single' ) {
					$thumb_size_meta = get_post_meta( $post->ID, '_ws_thumb', true );
				...
				if( $thumb_size_option ) {
					switch( $thumb_size_option ) {
						 case 'hide' :
						 	...
						 	break;
						 case 'full' :
						 	$sidebar_layout == 'full_width' ? $size = 'ws_large' : $size = 'ws_medium';
						 	break;
						 case 'small' :
						 	$size = 'ws_small';
						 	break;
		...
		if( $size == 'hide' )
			$size = null;
		if( $size == 'full' )
			$location == 'featured' || $sidebar_layout == 'full_width' ? $size = 'ws_large' : $size = 'ws_medium';
		if( $size == 'small' )
			$size = 'ws_small';
		...
		if( $link ) {
			...
			$thumb_link_meta = get_post_meta( $post->ID, '_ws_thumb_link', true );
			if( in_array( $thumb_link_meta, $possible_link_options ) ) {
				switch( $thumb_link_meta ) {
					...
					case 'thumbnail' :
						...
						$link_target = ' rel="featured_wordstrap_lightbox"';
						if( $gallery )
							$link_target = str_replace( 'featured_wordstrap_lightbox', 'featured_wordstrap_lightbox['.$gallery.']', $link_target );
						break;
					case 'image' :
						$link_url = get_post_meta( $post->ID, '_ws_image_link', true );
						$link_target = ' rel="featured_wordstrap_lightbox"';
						if( $gallery )
							$link_target = str_replace( 'featured_wordstrap_lightbox', 'featured_wordstrap_lightbox['.$gallery.']', $link_target );
						break;
					case 'video' :
						$link_url = get_post_meta( $post->ID, '_ws_video_link', true );
						$link_target = ' rel="featured_wordstrap_lightbox"';
						if( $gallery )
							$link_target = str_replace( 'featured_wordstrap_lightbox', 'featured_wordstrap_lightbox['.$gallery.']', $link_target );
						break;
					case 'external' :
						$link_url = get_post_meta( $post->ID, '_ws_external_link', true );
						$target = get_post_meta( $post->ID, '_ws_external_link_target', true );
						if( ! $target )
							$target = '_blank';
				...
				$end_link = apply_filters( 'wordstrap_image_overlay', $end_link );
			} else {
				$link = false;
			}
		}
		...
		// Attributes
		$size_class = $size;
		if( $size_class == 'ws_small' ) $size_class = 'small';
		...
		if( $allow_filters )
			$output = apply_filters( 'wordstrap_post_thumbnail', $output, $location, $size, $link );
...
if( ! function_exists( 'wordstrap_placeholder_image' ) ) {
	function wordstrap_placeholder_image( $size ) {
		$image_url = null;
		$images = array(
			'slider-large' 	=> WS_FRAMEWORK_DIRECTORY . '/frontend/assets/images/placeholders/slider-large.png',
			'slider-staged' => WS_FRAMEWORK_DIRECTORY . '/frontend/assets/images/placeholders/slider-large.png',
		);
		$images = apply_filters( 'wordstrap_placeholder_image', $images );
...
if( ! function_exists( 'wordstrap_remove_trailing_char' ) ) {
	function wordstrap_remove_trailing_char( $string, $char = ' ' ) {
...
if( ! function_exists( 'wordstrap_get_font_face' ) ) {
	function wordstrap_get_font_face( $option ) {
		$stack = null;
		$stacks = wordstrap_font_stacks();
		if( $option['face'] == 'google'  ) {
			...
			$name = wordstrap_remove_trailing_char( $name[0] ); 
...
if( ! function_exists( 'wordstrap_primary_menu_fallback' ) ) { 
	function wordstrap_primary_menu_fallback() {
		$home_text = wordstrap_get_local('home');
...
if( ! function_exists( 'wordstrap_query_string' ) ) { 
	function wordstrap_query_string( $posts_per_page = null ) {
		...
		if( is_home() || is_page_template( 'template_list.php' ) ) {
			if( ! $categories && ! $category_name_string ) {
				$exclude = wordstrap_get_option( 'blog_categories' );
				if( $exclude ) {
					$categories = 'cat=';
					foreach( $exclude as $key => $value )
						if( $value )
							$categories .= '-'.$key.',';
					$categories = wordstrap_remove_trailing_char( $categories, ',' );
				}
			}
		}
		// Categories (IDs)
		...
		// Return query string
		return apply_filters( 'wordstrap_query_string', $query_string );
	}
}
...
if( ! function_exists( 'wordstrap_get_posts_args' ) ) { 
	function wordstrap_get_posts_args( $options, $type, $slider = false ) {
		$args = array();
...
if( ! function_exists( 'wordstrap_posts_page_page' ) ) {
	function wordstrap_posts_page_page( $type, $columns = null, $rows = null ) {
		if( $type == 'template' ) {
			global $post;
			...
			if( ! in_array( intval($columns), $possible_column_nums ) )
				$columns = apply_filters( 'wordstrap_default_grid_columns', 3 );
			// Rows
			$rows = get_post_meta( $post->ID, 'rows', true );
			if( ! $rows )
				$rows = apply_filters( 'wordstrap_default_grid_columns', 4 );
		}
		// Posts per page
...
if( ! function_exists( 'wordstrap_grid_class' ) ) {
	function wordstrap_grid_class( $columns ) {
...
if( ! function_exists( 'wordstrap_open_row' ) ) {
	function wordstrap_open_row() {
		echo apply_filters( 'wordstrap_open_row', '<div class="grid-row">' );
	}
}
...
if( ! function_exists( 'wordstrap_close_row' ) ) {
	function wordstrap_close_row() {
		echo apply_filters( 'wordstrap_close_row', '<div class="clear"></div></div><!-- .grid-row (end) -->' );
	}
}
...
if( ! function_exists( 'wordstrap_oembed_result' ) ) {
	function wordstrap_oembed_result( $input, $url ) {
		...
		$filter_applied = strpos( $input, 'wordstrap' );
		...
		// Wrap output
		$output  = '<div class="wordstrap-'.$media.'-wrapper">';
		$output .= '<div class="'.$media.'-inner">';
		$output .= $input;
		$output .= '</div><!-- .video-inner (end) -->';
		$output .= '</div><!-- .wordstrap-video-wrapper (end) -->';
...
if( ! function_exists( 'wordstrap_tweet_filter_default' ) ) {
	function wordstrap_tweet_filter_default( $text, $username ) {
...
if( ! function_exists( 'wordstrap_feed_transient' ) ) {
	function wordstrap_feed_transient( $seconds ) {
...
if( ! function_exists( 'wordstrap_get_twitter_rss_url' ) ) {
	function wordstrap_get_twitter_rss_url( $username ) {
		...
		return apply_filters( 'wordstrap_twitter_rss', $url, $username );
	}
}
...
if( ! function_exists( 'wordstrap_analytics' ) ) {
	function wordstrap_analytics() {
		$analytics = wordstrap_get_option( 'analytics' );
		if( $analytics )
			echo $analytics;
	}
}
...
if( ! function_exists( 'wordstrap_text_color' ) ) {
	function wordstrap_text_color( $bg_color ) {
		...
		if( $r + $g + $b > 382 )
		    $text_color = apply_filters( 'wordstrap_dark_font', '#333333' ); // bright color, use dark font
		else
		    $text_color = apply_filters( 'wordstrap_light_font', '#ffffff' );; // dark color, use bright font
...
if( ! function_exists( 'wordstrap_adjust_color' ) ) {
	function wordstrap_adjust_color( $color, $difference = 20, $direction = 'darken' ) {
...
if( ! function_exists( 'wordstrap_get_classes' ) ) {
	function wordstrap_get_classes( $element, $start_space = false, $end_space = false, $type = null, $options = array() ) {
		$classes = '';
		$all_classes = array(
			'element_columns' 				=> '',
			'element_content' 				=> '',
		...
		$all_classes = apply_filters( 'wordstrap_element_classes', $all_classes, $type, $options );
...
if( ! function_exists( 'wordstrap_responsive_visibility_class' ) ) {
	function wordstrap_responsive_visibility_class( $devices, $start_space = false, $end_space = false ) {
		// Build class
		$class = '';
		...
		// Apply filter
		$class = apply_filters( 'wordstrap_responsive_visibility_class', $class, $devices );
		// Start/End spaces
		if( $class ) {
			if( $start_space )
				$class = ' '.$class;

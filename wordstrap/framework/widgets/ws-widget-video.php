<?php
/**
 * WordStrap Video Widget
 * 
 * @package WordStrap WordPress Framework
 * @author Jason Loftis
 */

class WS_Widget_Video extends WP_Widget {
	
	/* Constructor */
	
	function __construct() {
		$widget_ops = array(
			'classname' => 'ws-video_widget', 
        ...
        $this->WP_Widget( 'wordstrap_video_widget', 'WordStrap Video', $widget_ops, $control_ops );
	}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('video_url'); ?>"><?php _e('Video URL:', WS_GETTEXT_DOMAIN); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('description'); ?>"><?php _e('Description:', WS_GETTEXT_DOMAIN); ?></label>
	...
	function widget($args, $instance) {
		...
		if( ! $instance['video_url'] ) {
            _e('You forgot to enter a video URL.', WS_GETTEXT_DOMAIN );
            echo $after_widget;
            return;
		}
		echo wp_oembed_get( $instance['video_url'] );
		if( $instance['description'] )
			echo '<span class="ws-video_description">'.$instance['description'].'</span>';
		echo $after_widget;		
	}

}
register_widget('WS_Widget_Video');
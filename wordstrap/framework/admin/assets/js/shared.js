...
(function($){
	...
	// Setup methods for wordstrap namespace
	var wordstrap_shared = {
	...
	// Setup wordstrap namespace
	$.fn.wordstrap = function(method) {

		if( wordstrap_shared[method] )
		{
			return wordstrap_shared[ method ].apply( this, Array.prototype.slice.call( arguments, 1 ));
		}
		else if( typeof method === 'object' || ! method )
		{
			return wordstrap_shared.init.apply( this, arguments );
		}
		else
		{
...
(function ($) {
	tbc_alert = {
		init: function(alert_text, alert_class)
		{
			...
			// Initial HTML markup	
			var alert_markup	= '	<div id="ws-alert"> \
										<div class="ws-alert-inner">  \
											<div class="ws-alert-pad">  \
												<div class="ws-alert-message"> \
													<p>Replace this with message text.</p> \
												</div> \
											</div><!-- .ws-alert-pad (end) --> \
										</div><!-- .ws-alert-inner (end) --> \
									 </div><!-- .ws-alert (end) -->';
			...
			$this.addClass('ws-'+alert_class);
			$this.find('.ws-alert-message p').text(alert_text);
...
jQuery(document).ready(function($) {
    $('#wordstrap_theme_options .reset-button').click(function(){
		tbc_confirm('<h3>'+wordstrap.reset_title+'</h3>'+wordstrap.reset, {'confirm':true}, function(r)
		{
	    	if(r)
	        {
	        	// Add in reset so our sanitizaiton callback reconizes.
	        	$('#wordstrap_theme_options').append('<input type="hidden" name="reset" value="true" />');
	        	
	        	// Submit form
	        	$('#wordstrap_theme_options').submit();
	        }
	    });
	    return false;
	});
	
	$('#wordstrap_theme_options .clear-button').click(function(){
		tbc_confirm('<h3>'+wordstrap.clear_title+'</h3>'+wordstrap.clear, {'confirm':true}, function(r)
		{
	    	if(r)
	        {
	        	// Add in reset so our sanitizaiton callback reconizes.
	        	$('#wordstrap_theme_options').append('<input type="hidden" name="clear" value="true" />');

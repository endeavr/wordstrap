...
function sidebar_ws_rolescheck () {
	if ( wordstrap_supports( 'admin', 'sidebars' ) && current_user_can( wordstrap_admin_module_cap( 'sidebars' ) ) ) {
		// If the user can edit theme options, let the fun begin!
		add_action( 'admin_menu', 'sidebar_ws_add_page' );
		add_action( 'admin_init', 'sidebar_ws_hijack_submenu' );
		add_action( 'admin_init', 'sidebar_ws_init' );
		add_action( 'widgets_admin_page', 'sidebar_ws_widgets_admin_page' );
	}
}
add_action( 'init', 'sidebar_ws_rolescheck' );
...
if( ! function_exists( 'sidebar_ws_init' ) ) {	
	function sidebar_ws_init() {
...
if ( ! function_exists( 'sidebar_ws_add_page' ) ) {
	function sidebar_ws_add_page() {
		
		$title = __( 'Widget Areas', WS_GETTEXT_DOMAIN );
		$bb_page = add_theme_page( $title, $title, wordstrap_admin_module_cap( 'sidebars' ), 'sidebar_ws', 'sidebar_ws_page' );
		...
		add_action( 'admin_print_styles-'.$bb_page, 'sidebar_ws_load_styles' );
		add_action( 'admin_print_scripts-'.$bb_page, 'sidebar_ws_load_scripts' );
...
if ( ! function_exists( 'sidebar_ws_hijack_submenu' ) ) {
	function sidebar_ws_hijack_submenu() {
		global $submenu;
		$new_submenu = array();
		if( $submenu ) {
			// Find the current "Widget Areas" array, copy it, and remove it.
			foreach( $submenu['themes.php'] as $key => $value ) {
				if( $value[2] == 'sidebar_ws' ) {
					$widget_areas = $value;
...
if ( ! function_exists( 'sidebar_ws_load_styles' ) ) {
	function sidebar_ws_load_styles() {
...
if ( ! function_exists( 'sidebar_ws_load_scripts' ) ) {
	function sidebar_ws_load_scripts() {
...
if ( ! function_exists( 'sidebar_ws_widgets_admin_page' ) ) {
	function sidebar_ws_widgets_admin_page() {
		...
		_e( 'In the <a href="themes.php?page=sidebar_ws">Widget Area Manager</a>, you can create and manage widget areas for specific pages of your website to override the default locations you see below.', WS_GETTEXT_DOMAIN);
		echo '</p></div>';
	}
} 
...
if ( ! function_exists( 'sidebar_ws_page_header' ) ) {
	function sidebar_ws_page_header() {
		?>
		<div id="sidebar_ws">
			<div id="optionsframework" class="wrap">
			    <div class="admin-module-header">
			    	<?php do_action( 'wordstrap_admin_module_header', 'sidebars' ); ?>
			    </div>
			    <?php screen_icon( 'themes' ); ?>
			    <h2 class="nav-tab-wrapper">
			        <a href="#manage_sidebars" id="manage_sidebars-tab" class="nav-tab" title="<?php _e( 'Custom Widget Areas', WS_GETTEXT_DOMAIN ); ?>"><?php _e( 'Custom Widget Areas', WS_GETTEXT_DOMAIN ); ?></a>
			        <a href="#add_sidebar" id="add_sidebar-tab" class="nav-tab" title="<?php _e( 'Add New', WS_GETTEXT_DOMAIN ); ?>"><?php _e( 'Add New', WS_GETTEXT_DOMAIN ); ?></a>
			        <a href="#edit_sidebar" id="edit_sidebar-tab" class="nav-tab nav-edit-sidebar" title="<?php _e( 'Edit', WS_GETTEXT_DOMAIN ); ?>"><?php _e( 'Edit', WS_GETTEXT_DOMAIN ); ?></a>
			    </h2>
	    <?php
	}	
}
...
if ( ! function_exists( 'sidebar_ws_page_footer' ) ) {
	function sidebar_ws_page_footer() {
		?>
				<div class="admin-module-footer">
					<?php do_action( 'wordstrap_admin_module_footer', 'sidebars' ); ?>
				</div>
...
if ( ! function_exists( 'sidebar_ws_page' ) ) {
	function sidebar_ws_page() {
		sidebar_ws_page_header();
		?>
    	...
    	<div id="manage_sidebars" class="group">
	    	<form id="manage_current_sidebars">	
				...
				?>
				<div class="ajax-mitt"><?php sidebar_ws_manage(); ?></div>

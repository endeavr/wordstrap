...
function slider_ws_rolescheck () {
	if ( wordstrap_supports( 'admin', 'sliders' ) && current_user_can( wordstrap_admin_module_cap( 'sliders' ) ) ) {
		add_action( 'admin_menu', 'slider_ws_add_page');
		add_action( 'admin_init', 'slider_ws_init' );
	}
}
add_action( 'init', 'slider_ws_rolescheck' );
...
if( ! function_exists( 'slider_ws_init' ) ) {
	function slider_ws_init() {
...
if( ! function_exists( 'slider_ws_slider_defaults' ) ) {
	function slider_ws_slider_defaults( $type ) {
		
		$default_options = array();
		$sliders = slider_ws_recognized_sliders();
...
if ( ! function_exists( 'slider_ws_add_page' ) ) {
	function slider_ws_add_page() {
		
		$icon = WORDSTRAP_ADMIN_ASSETS_DIRECTORY . 'images/icon-images.png';
		$sb_page = add_object_page( 'Slider Manager', 'Sliders', wordstrap_admin_module_cap( 'sliders' ), 'slider_ws', 'slider_ws_page', $icon, 31 );
		...
		add_action( 'admin_print_styles-'.$sb_page, 'slider_ws_load_styles' );
		add_action( 'admin_print_scripts-'.$sb_page, 'slider_ws_load_scripts' );
...
if( ! function_exists( 'slider_ws_load_styles' ) ) {
	function slider_ws_load_styles() {
...
if( ! function_exists( 'slider_ws_load_scripts' ) ) {
	function slider_ws_load_scripts() {
 ...
 * slide when using the slider_ws_edit_slide() function.
 *
 * @since 2.0.0
 *
...
if( ! function_exists( 'slider_ws_slide_value' ) ) {
	function slider_ws_slide_value( $slide_options, $type, $sub_type = null ) {
...
if ( ! function_exists( 'slider_ws_page_header' ) ) {
	function slider_ws_page_header() {
		?>
		<div id="slider_ws">
			<div id="optionsframework" class="wrap">
			    <div class="admin-module-header">
			    	<?php do_action( 'wordstrap_admin_module_header', 'sliders' ); ?>
			    </div>
			    <?php screen_icon( 'themes' ); ?>
			    <h2 class="nav-tab-wrapper">
			        <a href="#manage" id="manage-tab" class="nav-tab" title="<?php _e( 'Manage Sliders', WS_GETTEXT_DOMAIN ); ?>"><?php _e( 'Manage Sliders', WS_GETTEXT_DOMAIN ); ?></a>
			        <a href="#add" id="add-tab" class="nav-tab" title="<?php _e( 'Add New Slider', WS_GETTEXT_DOMAIN ); ?>"><?php _e( 'Add Slider', WS_GETTEXT_DOMAIN ); ?></a>
			        <a href="#edit" id="edit-tab" class="nav-tab nav-edit-slider" title="<?php _e( 'Edit Slider', WS_GETTEXT_DOMAIN ); ?>"><?php _e( 'Edit Slider', WS_GETTEXT_DOMAIN ); ?></a>
			    </h2>
	    <?php
	}	
}
...
if ( ! function_exists( 'slider_ws_page_footer' ) ) {
	function slider_ws_page_footer() {
		?>
				<div class="admin-module-footer">
					<?php do_action( 'wordstrap_admin_module_footer', 'sliders' ); ?>
				</div>
...
if ( ! function_exists( 'slider_ws_page' ) ) {
	function slider_ws_page() {
		$types = slider_ws_recognized_sliders();
		slider_ws_page_header();
		?>
    	<!-- MANAGE SLIDER (start) -->
    	...
    	<div id="manage" class="group">
	    	<form id="manage_sliders">	
				...
				?>
				<div class="ajax-mitt"><?php slider_ws_manage(); ?></div>

<?php
// Hooks
add_action( 'wp_ajax_slider_ws_add_slider', 'slider_ws_ajax_add_slider' );
add_action( 'wp_ajax_slider_ws_save_slider', 'slider_ws_ajax_save_slider' );
add_action( 'wp_ajax_slider_ws_add_slide', 'slider_ws_ajax_add_slide' );
add_action( 'wp_ajax_slider_ws_update_table', 'slider_ws_ajax_update_table' );
add_action( 'wp_ajax_slider_ws_delete_slider', 'slider_ws_ajax_delete_slider' );
add_action( 'wp_ajax_slider_ws_edit_slider', 'slider_ws_ajax_edit_slider' );
...
if( ! function_exists( 'slider_ws_ajax_add_slider' ) ) {
	function slider_ws_ajax_add_slider () {
		...
		$options = slider_ws_slider_defaults( $config['options']['slider_type'] );
...
if( ! function_exists( 'slider_ws_ajax_save_slider' ) ) {
	function slider_ws_ajax_save_slider () {
		...
		$sb_sliders = slider_ws_recognized_sliders();
		...
		if( isset( $data['slides'] ) ) {
			...
			if( ! empty( $slides ) ) {
				foreach( $slides as $key => $slide ) {
					...
					if( isset( $slides[$key]['image']['id'] ) ) {
						$medium = wp_get_attachment_image_src( $slides[$key]['image']['id'], 'ws_medium' );
						$slides[$key]['image']['ws_medium'] = $medium[0];
						
						$small = wp_get_attachment_image_src( $slides[$key]['image']['id'], 'ws_small' );
						$slides[$key]['image']['ws_small'] = $small[0];
		...
		do_action( 'wordstrap_save_slider_'.$slider_type, $slider_id, $slides, $settings );
		
		// Display update message
		echo '<div id="setting-error-save_options" class="updated fade settings-error ajax-update">';
		echo '	<p><strong>'.__( 'Slider saved.', WS_GETTEXT_DOMAIN ).'</strong></p>';
		echo '</div>';
		die();
	}
}
...
if( ! function_exists( 'slider_ws_ajax_add_slide' ) ) {
	function slider_ws_ajax_add_slide() {	
		$atts = explode( '=>', $_POST['data'] );
		$slide_id = uniqid( 'slide_'.rand() );
		slider_ws_edit_slide( $atts[0], $atts[1], $slide_id, null, 'hide' );
		die();
	}
}
...
if( ! function_exists( 'slider_ws_ajax_update_table' ) ) {
	function slider_ws_ajax_update_table() {
		slider_ws_manage();
		die();
	}
}
...
if( ! function_exists( 'slider_ws_ajax_delete_slider' ) ) {
	function slider_ws_ajax_delete_slider() {
		...
		if( isset( $data['posts'] ) ) {
			...
			$posts = get_posts( array( 'post_type' => 'ws_slider', 'numberposts' => -1 ) );
			...
			echo '	<p><strong>'.__( 'Slider(s) deleted.', WS_GETTEXT_DOMAIN ).'</strong></p>';
			echo '</div>';
...
if( ! function_exists( 'slider_ws_ajax_edit_slider' ) ) {
	function slider_ws_ajax_edit_slider() {
		$slider_id = $_POST['data'];
		$types = slider_ws_recognized_sliders();
		echo $slider_id.'[(=>)]';
		slider_ws_edit( $_POST['data'], $types );
		die();
	}
}

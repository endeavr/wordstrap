...
if( ! function_exists( 'builder_ws_samples' ) ) {
	function builder_ws_samples() {
		...
		$imgpath = WS_FRAMEWORK_DIRECTORY.'/frontend/assets/images/sample';
		...
		$samples = array_merge( $samples, $_wordstrap_user_sample_layouts );
		
		// Remove sample layouts
		if( $_wordstrap_remove_sample_layouts )
			foreach( $_wordstrap_remove_sample_layouts as $layout )
				unset( $samples[$layout] );
		
		// Return with filters applied
		return apply_filters( 'wordstrap_sample_layouts', $samples );
	}	
}
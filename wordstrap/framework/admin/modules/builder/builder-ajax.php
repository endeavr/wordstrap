<?php
// Hooks
add_action( 'wp_ajax_builder_ws_add_layout', 'builder_ws_ajax_add_layout' );
add_action( 'wp_ajax_builder_ws_save_layout', 'builder_ws_ajax_save_layout' );
add_action( 'wp_ajax_builder_ws_add_element', 'builder_ws_ajax_add_element' );
add_action( 'wp_ajax_builder_ws_update_table', 'builder_ws_ajax_update_table' );
add_action( 'wp_ajax_builder_ws_delete_layout', 'builder_ws_ajax_delete_layout' );
add_action( 'wp_ajax_builder_ws_edit_layout', 'builder_ws_ajax_edit_layout' );
...
if( ! function_exists( 'builder_ws_ajax_add_layout' ) ) {
	function builder_ws_ajax_add_layout() {
...
if( ! function_exists( 'builder_ws_ajax_save_layout' ) ) {
	function builder_ws_ajax_save_layout () {
		...
		if( isset( $data['elements'] ) ) {
			...
			$default_element_options = wordstrap_get_elements();
		...
		echo '	<p><strong>'.__( 'Layout saved.', WS_GETTEXT_DOMAIN ).'</strong></p>';
		echo '</div>';
		die();
	}
}
...
if( ! function_exists( 'builder_ws_ajax_add_element' ) ) {
	function builder_ws_ajax_add_element() {	
		$element_type = $_POST['data'];
		$element_id = uniqid( 'element_'.rand() );
		builder_ws_edit_element( $element_type, $element_id );
		die();
	}
}
...
if( ! function_exists( 'builder_ws_ajax_update_table' ) ) {
	function builder_ws_ajax_update_table() {
		builder_ws_manage();
		die();
	}
}
...
if( ! function_exists( 'builder_ws_ajax_delete_layout' ) ) {
	function builder_ws_ajax_delete_layout() {
		...
		if( isset( $data['posts'] ) ) {
			...
			$posts = get_posts( array( 'post_type' => 'ws_layout', 'numberposts' => -1 ) );
			...
			echo '	<p><strong>'.__( 'Layout(s) deleted.', WS_GETTEXT_DOMAIN ).'</strong></p>';
			echo '</div>';
...
if( ! function_exists( 'builder_ws_ajax_edit_layout' ) ) {
	function builder_ws_ajax_edit_layout() {
		$layout_id = $_POST['data'];
		echo $layout_id.'[(=>)]';
		builder_ws_edit( $layout_id );
		die();
	}
}
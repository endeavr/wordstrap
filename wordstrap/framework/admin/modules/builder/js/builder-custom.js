...
jQuery(document).ready(function($) {
	...
	var builder_ws = {
    	...
    	manager : function( table )
    	{
    		if(table)
			{
				...
				$('#builder_ws #manage_layouts .ajax-mitt').html(table);
			}
			else
			{
				...
				$.ajax({
					...
					data:
					{
						action: 'builder_ws_update_table'
					},
					success: function(response)
					{	
						$('#builder_ws #manage_layouts .ajax-mitt').html(response);
					}
				});
			}
    	},
    	...
    	delete_layout : function( ids, action, location )
    	{
			...
			tbc_confirm( wordstrap.delete_layout, {'confirm':true}, function(r)
			{
		    	if(r)
		        {
		        	$.ajax({
						...
						data:
						{
							action: 'builder_ws_delete_layout',
							security: nonce,
							data: ids
						},
						success: function(response)
						{	
							...
							// after a few seconds.
							$('#builder_ws #manage_layout').prepend(response[1]);
							$('#builder_ws #manage_layout .ajax-update').fadeIn(500, function(){
								setTimeout( function(){
									$('#builder_ws #manage_layout .ajax-update').fadeOut(500, function(){
										$('#builder_ws #manage_layout .ajax-update').remove();
									});
						      	}, 1500);
							...
							$('#builder_ws .displaying-num').text(response[0]);
							...
							{
								$('#builder_ws .group').hide();
								$('#builder_ws .group:first').fadeIn();
								$('#builder_ws .nav-tab-wrapper a:first').addClass('nav-tab-active');
							}
						}
					});
		        }
    	...
    	edit : function ( name, page )
    	{
			...
			$('#builder_ws .nav-tab-wrapper a.nav-edit-builder').text(wordstrap.edit_layout+': '+name).addClass(page[0]+'-edit');
			$('#builder_ws #edit_layout .ajax-mitt').html(page[1]);
			...
			// Setup widgets
			$('#builder_ws .widget').wordstrap('widgets');
			
			// Setup options
			$('#builder_ws').wordstrap('options', 'setup');
			
			// Take us to the tab
			$('#builder_ws .nav-tab-wrapper a').removeClass('nav-tab-active');
			$('#builder_ws .nav-tab-wrapper a.nav-edit-builder').show().addClass('nav-tab-active');
			$('#builder_ws .group').hide();
			$('#builder_ws .group:last').fadeIn();
	...
	$('#builder_ws .nav-tab-wrapper a.nav-edit-builder').hide();
	...
	$('#builder_ws #manage_layouts .edit-ws_layout').live( 'click', function(){
		...
		$.ajax({
			...
			data:
			{
				action: 'builder_ws_edit_layout',
				data: id
			},
			success: function(response)
			{	
				builder_ws.edit( name, response );
			}
		});
		return false;
	});
	...
	$('#builder_ws .row-actions .trash a').live( 'click', function(){
		var href = $(this).attr('href'), id = href.replace('#', ''), ids = 'posts%5B%5D='+id;
		builder_ws.delete_layout( ids, 'click' );
		return false;
	});
	...
	$('#manage_builder').live( 'submit', function(){
		...
		if(value == 'trash')
		{
			builder_ws.delete_layout( ids, 'submit' );
		}
		return false;
	});
		...
		$.ajax({
			...
			{	
				...
				$('body').animate( { scrollTop: 0 }, 100, function(){						
					...
					builder_ws.edit( name, response );
					tbc_alert.init(wordstrap.layout_created, 'success');
	...
	$('#optionsframework #add_new_element').live( 'click', function(){
		...
		if(query == 'primary')
		{
			...
			$('#builder_ws #builder .element-query').each(function(){
				if( $(this).val() == 'primary' )
				{
					primary_query = true;
				}
			...
			if(primary_query)
			{
				...
				tbc_confirm(wordstrap.primary_query, {'textOk':'Ok'});
				return false;
			}
		}
		...
		$.ajax({
			...
			{	
				...
				$('#builder_ws #edit_layout #primary .sortable').append(response);
				$('#builder_ws #edit_layout #primary .sortable').removeClass('empty');
				$('#'+element_id).wordstrap('widgets');
				$('#'+element_id).wordstrap('options', 'setup');
				$('#'+element_id).fadeIn();									
				load.fadeOut('fast');
	...
	$('#optionsframework #edit_builder').live('submit', function(){
		...
		$.ajax({
			...
			{	
				// Insert update message, fade it in, and then remove it 
				// after a few seconds.
				$('#builder_ws #edit_layout').prepend(response);
				
				// Scroll to top of page
				$('body').animate( { scrollTop: 0 }, 50, function(){						
					// Fade in the update message
					$('#builder_ws #edit_layout .ajax-update').fadeIn(500, function(){
						setTimeout( function(){
							$('#builder_ws #edit_layout .ajax-update').fadeOut(500, function(){
								$('#builder_ws #edit_layout .ajax-update').remove();
							});
				      	}, 1500);
	...
	$('#builder_ws #edit_layout .delete_layout').live( 'click', function(){
		var href = $(this).attr('href'), id = href.replace('#', ''), ids = 'posts%5B%5D='+id;
		builder_ws.delete_layout( ids, 'click', 'edit_page' );
		return false;
	});

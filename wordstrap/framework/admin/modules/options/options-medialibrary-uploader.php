...
if ( ! function_exists( 'optionsframework_mlu_init' ) ) {
	function optionsframework_mlu_init () {
		register_post_type( 'optionsframework', array(
			'labels' => array(
				'name' => __( 'Options Framework Internal Container', WS_GETTEXT_DOMAIN ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'optionsframework_mlu_css' ) ) {
	...
	function optionsframework_mlu_css () {
		...
		var ws_pathToImage = "' . get_option('siteurl') . '/' . WPINC . '/js/thickbox/loadingAnimation.gif";
	    var ws_closeImage = "' . get_option('siteurl') . '/' . WPINC . '/js/thickbox/ws-close.png";
	    </script>' . "\n";
...
if ( ! function_exists( 'optionsframework_mlu_js' ) ) {
	...
	function optionsframework_mlu_js () {
		...
		wp_register_script( 'of-medialibrary-uploader', WS_OPTIONS_FRAMEWORK_DIRECTORY .'js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
 ...
 *
 * Parameters:
 * - string $option_name - Required prefix for form name attributes (Added by WordStrap) 
 * - string $type - Required type of media uploader, standard or slider (Added by WordStrap) 
 ...
 * - string $_upload_text - An optional text string for the "Upload" button. (Added by WordStrap)
 *
 * Dependencies:
...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	...
	function optionsframework_medialibrary_uploader( $option_name, $type, $_id, $_value, $_mode = 'full', $_desc = null, $_postid = 0, $_name = null, $_upload_text = null) {
		...
		$int = '';
		$value = array( 'url' => '', 'id' => '' );
		$upload_text = __( 'Upload', WS_GETTEXT_DOMAIN );
		...
		if( $type == 'slider' ) {
			...
			$output .= '<p class="explain">'.__( 'You must use the "Get Image" button modify the image ID for this slide. This is what the locked icon represents.', WS_GETTEXT_DOMAIN ).'</p>';
		} else {
		...
		if ( $value['url'] != '' ) { 
			...
			if ( $image ) {
				...
				$title = __( 'View File', WS_GETTEXT_DOMAIN );
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) {
	...
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', WS_GETTEXT_DOMAIN ), __( 'Previously Uploaded', WS_GETTEXT_DOMAIN ), $tabs['gallery'] );
		return $tabs;
	}
}
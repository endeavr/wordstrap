...
function ws_optionsframework_fields( $option_name, $options, $settings, $close = true ) {
	...
	foreach ($options as $value) {
		...
		// common groups.
	   	...
	   	// Name Grouping (added by WordStrap)
		...
		if ( ($value['type'] != "heading") && ($value['type'] != "info") ) {
			...
			if( isset( $value['name'] ) ) // Name not required any more (edit by WordStrap)
		...
		// If the option is already saved, ovveride $val (Modified by WordStrap to check for grouping)
		if ( ($value['type'] != 'heading') && ($value['type'] != 'info')) {
			if( isset( $value['group'] ) ) {
				// Set grouped value
			...
			foreach ($value['options'] as $key => $option ) {
			 ...
			 if( isset( $value['class'] ) && $value['class'] == 'builder_samples' ) {
			 	if( function_exists( 'builder_ws_sample_previews' ) ) {
			 		$output .= builder_ws_sample_previews();	
			 	}
			 }
		...
		// Uploader
		case "upload":
			$val = array( 'url' => $val, 'id' => '' ); // Added by WordStrap
			$output .= optionsframework_medialibrary_uploader( $option_name, 'standard', $value['id'], $val ); // New AJAX Uploader using Media Library // Mod'd by WordStrap	
		break;
		...
		case 'typography':	
			...
			*/
			...
			// Google Font support (added by WordStrap)
			if( in_array( 'face', $value['atts'] ) ) {
				$output .= '<div class="google-font hide">';
				$output .= '<h5>'.__( 'Enter the name of a font from the <a href="http://www.google.com/webfonts" target="_blank">Google Font Directory</a>.', WS_GETTEXT_DOMAIN ).'</h5>';
		...
		case 'background':
			...
			$current_bg_image = array( 'url' => $background['image'], 'id' => '' ); // Added by WordStrap
			$output .= optionsframework_medialibrary_uploader( $option_name, 'standard', $value['id'], $current_bg_image, null, '',0,'image'); // Mod'd by WordStrap
		...
		// Columns Setup (added by WordStrap)
		case "columns":
			$output .= wordstrap_columns_option( $value['options'], $value['id'], $option_name, $val );
		break;
		
		// Tabs Setup (added by WordStrap)
		case "tabs":
			$output .= wordstrap_tabs_option( $value['id'], $option_name, $val );
		break;
		
		// Content (added by WordStrap)
		// Originally designed to work in conjunction with setting up columns and tabs
		case "content":
			$output .= wordstrap_content_option( $value['id'], $option_name, $val, $value['options'] );
		break;
		
		// Conditionals (added by WordStrap)
		// Originally designed to allow users to assign custom sidebars to certain pages.
		case "conditionals":
			$output .= wordstrap_conditionals_option( $value['id'], $option_name, $val );
		break; 
		
		// Logo (added by WordStrap)
		case "logo":
			$output .= wordstrap_logo_option( $value['id'], $option_name, $val );
		break; 
		
		// Social Media (added by WordStrap)
		case "social_media":
			$output .= wordstrap_social_media_option( $value['id'], $option_name, $val );
		break;                       

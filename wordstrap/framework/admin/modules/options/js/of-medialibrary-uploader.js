...
/*-----------------------------------------------------------------------------------*/
    ...
    removeFile: function () {
     ...
     $('.mlu_remove').live('click', function(event) { 
        var el = $(this);
        el.hide();
        el.closest('.section-upload').find('.upload').attr('value', ''); // Mod'd to find ALL .upload (by WordStrap)
...
/*-----------------------------------------------------------------------------------*/
	...
	$('input.upload_button').live("click", function () {
        formfield = $(this).closest('.section-upload').find('input.image-url').attr('id'); // Mod'd to allow more variations of HTML markup (by WordStrap)
        formID = $(this).attr('rel'),
        upload_type = $(this).attr('data-upload-type'); // Added by WordStrap
        
		// Change "insert into post" to "Use this Button" // ... Altered for different upload types (Mod by WordStrap)
		if( upload_type == 'slider' )
		{
			tbframe_interval = setInterval(function()
			{
				jQuery('#WS_iframeContent').contents().find('.savesend .button').val('Insert into Slider');
				jQuery('#WS_iframeContent').contents().find('.slidetoggle tr.image_alt, .slidetoggle tr.post_excerpt, .slidetoggle tr.post_content, .slidetoggle tr.url, .slidetoggle tr.align, .slidetoggle tr.image-size').hide();
			}, 2000);
        }
        else if( upload_type == 'logo' )
        {
        	tbframe_interval = setInterval(function() {jQuery('#WS_iframeContent').contents().find('.savesend .button').val('Use as Logo');}, 2000);
        }
        else
        {
        	tbframe_interval = setInterval(function() {jQuery('#WS_iframeContent').contents().find('.savesend .button').val('Use This Image');}, 2000);
        }
		...
		ws_show( woo_title, 'media-upload.php?post_id='+formID+'&wordstrap_upload_type='+upload_type+'&WS_iframe=1' ); // Modded to pass in upload_type by WordStrap
		return false;
	});
	...
	window.send_to_editor = function(html) {
		...
		if (formfield) {
			...
			// Add media item's ID in the mix if this is for a slider (Mod by WordStrap)
			if( upload_type == 'slider' )
			{

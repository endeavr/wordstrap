...
function wordstrap_post_table( $post_type, $columns ) {
	...
	if( $post_type == 'ws_sidebar' )
		$conflicts = wordstrap_get_assignment_conflicts( $posts );
	...
	$output .= '<option value="-1" selected="selected">'.__( 'Bulk Actions', WS_GETTEXT_DOMAIN ).'</option>';
	$output .= '<option value="trash">'.__( 'Delete', WS_GETTEXT_DOMAIN ).' '.$name.'</option>';
	$output .= '</select>';
	$output .= '<input type="submit" id="doaction" class="button-secondary action" value="'.__( 'Apply', WS_GETTEXT_DOMAIN ).'">';
	$output .= '</div>';
	...
	if( ! empty( $posts ) ) {
			...
			foreach( $columns as $column ){
				switch( $column['type'] ) {
					case 'title' :
						...
						$output .= '<strong><a href="#'.$post->ID.'" class="title-link edit-'.$post_type.'" title="'.__( 'Edit', WS_GETTEXT_DOMAIN ).'">'.stripslashes($post->post_title).'</strong></a>';
						$output .= '<div class="row-actions">';
						$output .= '<span class="edit">';
						$output .= '<a href="#'.$post->ID.'" class="edit-post edit-'.$post_type.'" title="'.__( 'Edit', WS_GETTEXT_DOMAIN ).'">'.__( 'Edit', WS_GETTEXT_DOMAIN ).'</a> | ';
						$output .= '</span>';
						$output .= '<span class="trash">';
						$output .= '<a title="'.__( 'Delete', WS_GETTEXT_DOMAIN ).'" href="#'.$post->ID.'">'.__( 'Delete', WS_GETTEXT_DOMAIN ).'</a>';
						$output .= '</span>';
						$output .= '</div>';
						break;
					...
					case 'assignments' :
						...
						if( $location && $location != 'floating' ) {
							...
							if( is_array( $assignments ) && ! empty( $assignments ) ) {
								...
								$output .= '<span class="inactive">'.__( 'No Assignments', WS_GETTEXT_DOMAIN ).'</span>';
							}
						} else {
					...
					case 'sidebar_location' :
						...
						$output .= wordstrap_get_sidebar_location_name( get_post_meta( $post->ID, 'location', true ) );
						break;
				}
				$output .= '</td>';
			}
		...
		$output .= '<tr><td colspan="'.$num.'">'.__('No items have been created yet. Click the Add tab above to get started.', WS_GETTEXT_DOMAIN).'</td></tr>';
	}
	$output .= '</tbody>';
	$output .= '</table>';
	return $output;
...
function wordstrap_columns_option( $type, $id, $name, $val ) {
	...
	$data_num = array (
		array(
			'name' 	=> __( 'Hide Columns', WS_GETTEXT_DOMAIN ),
			'value' => 0,
		),
		array(
			'name' 	=> '1 '.__( 'Column', WS_GETTEXT_DOMAIN ),
			'value' => 1,
		),
		array(
			'name' 	=> '2 '.__( 'Columns', WS_GETTEXT_DOMAIN ),
			'value' => 2,
		),
		array(
			'name' 	=> '3 '.__( 'Columns', WS_GETTEXT_DOMAIN ),
			'value' => 3,
		),
		array(
			'name' 	=> '4 '.__( 'Columns', WS_GETTEXT_DOMAIN ),
			'value' => 4,
		),
		array(
			'name' 	=> '5 '.__( 'Columns', WS_GETTEXT_DOMAIN ),
			'value' => 5,
		)
	);
	
	// Dropdowns for column width configuration
	$data_widths = wordstrap_column_widths();
	...
	$i = 1;
	$select_widths = '<div class="column-width column-width-0"><p class="inactive">'.__( 'Columns will be hidden.', WS_GETTEXT_DOMAIN ).'</p></div>';
...
function wordstrap_tabs_option( $id, $name, $val ) {
	...
	$numbers = array (
		array(
			'name' 	=> '2 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 2,
		),
		array(
			'name' 	=> '3 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 3,
		),
		array(
			'name' 	=> '4 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 4,
		),
		array(
			'name' 	=> '5 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 5,
		),
		array(
			'name' 	=> '6 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 6,
		),
		array(
			'name' 	=> '7 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 7,
		),
		array(
			'name' 	=> '8 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 8,
		),
		array(
			'name' 	=> '9 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 9,
		),
		array(
			'name' 	=> '10 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 10,
		),
		array(
			'name' 	=> '11 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 11,
		),
		array(
			'name' 	=> '12 '.__( 'Tabs', WS_GETTEXT_DOMAIN ),
			'value' => 12,
		)
	);
	...
	$select_style .= '<option value="open" '.selected( $current_value, 'open', false ).'>'.__( 'Open Style', WS_GETTEXT_DOMAIN ).'</option>';
	$select_style .= '<option value="framed" '.selected( $current_value, 'framed', false ).'>'.__( 'Framed Style', WS_GETTEXT_DOMAIN ).'</option>';
	$select_style .= '</select>';
	...
	foreach( $numbers as $number ) {
		...
		$input_names .= '<label for="tab-name-input='.$number['value'].'">'.sprintf( __('Tab #%d Name', WS_GETTEXT_DOMAIN), $number['value'] ).'</label>';
...
function wordstrap_content_option( $id, $name, $val, $options ) {
	...
	$sources = array(
		'null' 		=> __( '- Select Content Type -', WS_GETTEXT_DOMAIN ),
	);
	
	if( in_array ( 'widget', $options ) )
		$sources['widget'] = __( 'Floating Widget Area', WS_GETTEXT_DOMAIN );
		
	if( in_array ( 'page', $options ) )
		$sources['page'] = __( 'External Page', WS_GETTEXT_DOMAIN );
	
	if( in_array ( 'raw', $options ) )
		$sources['raw'] = __( 'Raw Content', WS_GETTEXT_DOMAIN );
	...
	if( in_array ( 'widget', $options ) ) {
		...
		if( ! empty( $sidebars ) ) {
			...
			$select_sidebar = '<p class="warning">'.__( 'You haven\'t created any floating widget areas.', WS_GETTEXT_DOMAIN ).'</p>';
		}
	...
	if( in_array ( 'page', $options ) ) {
		...
		$pages = wordstrap_get_select( 'pages' );
		...
		if( ! empty( $pages ) ) {
			...
			$select_page = '<p class="warning">'.__( 'You haven\'t created any pages.', WS_GETTEXT_DOMAIN ).'</p>';
		}
	...
	if( in_array ( 'raw', $options ) ) {
		...
		$raw_content .= '<input class="checkbox of-input" type="checkbox" name="'.esc_attr( $name.'['.$id.'][raw_format]' ).'"'.$checked.'>'.__( 'Apply WordPress automatic formatting.', WS_GETTEXT_DOMAIN );
	}
	...
	if( in_array ( 'widget', $options ) ) {
		...
		$output .= '<p class="note">'.__( 'Select from your floating custom widget areas. In order for a custom widget area to be "floating" you must have it configured this way in the Widget Area manager.', WS_GETTEXT_DOMAIN ).'</p>';
		$output .= '</div>';
	}
	...
	if( in_array ( 'page', $options ) ) {
		...
		$output .= $select_page;
		$output .= '<p class="note">'.__( 'Select an external page to pull content from.', WS_GETTEXT_DOMAIN ).'</p>';
		$output .= '</div>';
	}
	...
	if( in_array ( 'raw', $options ) ) {
		...
		$output .= $raw_content;
		$output .= '<p class="note">'.__( 'You can use basic HTML here, and most shortcodes.', WS_GETTEXT_DOMAIN ).'</p>';
		$output .= '</div>';
	}
...
function wordstrap_conditionals_option( $id, $name, $val = null ) {
...
function wordstrap_logo_option( $id, $name, $val ) {
	...
	$types = array(
		'title' 		=> __( 'Site Title', WS_GETTEXT_DOMAIN ),
		'title_tagline' => __( 'Site Title + Tagline', WS_GETTEXT_DOMAIN ),
		'custom' 		=> __( 'Custom Text', WS_GETTEXT_DOMAIN ),
		'image' 		=> __( 'Image', WS_GETTEXT_DOMAIN )
	);
	...
	$site_title .= __( 'Current Site Title', WS_GETTEXT_DOMAIN ).': <strong>';
	$site_title .= get_bloginfo( 'name' ).'</strong><br><br>';
	$site_title .= __( 'You can change your site title and tagline by going <a href="options-general.php" target="_blank">here</a>.', WS_GETTEXT_DOMAIN );
	$site_title .= '</p>';
	...
	$site_title_tagline .= __( 'Current Site Title', WS_GETTEXT_DOMAIN ).': <strong>';
	$site_title_tagline .= get_bloginfo( 'name' ).'</strong><br>';
	$site_title_tagline .= __( 'Current Tagline', WS_GETTEXT_DOMAIN ).': <strong>';
	$site_title_tagline .= get_bloginfo( 'description' ).'</strong><br><br>';
	$site_title_tagline .= __( 'You can change your site title by going <a href="options-general.php" target="_blank">here</a>.', WS_GETTEXT_DOMAIN );
	...
	$custom_text  = '<p><label class="inner-label"><strong>'.__( 'Title', WS_GETTEXT_DOMAIN ).'</strong></label>';
	$custom_text .= '<input type="text" name="'.esc_attr( $name.'['.$id.'][custom]' ).'" value="'.esc_attr($current_value).'" /></p>';
	$custom_text .= '<p><label class="inner-label"><strong>'.__( 'Tagline', WS_GETTEXT_DOMAIN ).'</strong> ('.__( 'optional', WS_GETTEXT_DOMAIN ).')</label>';
	$custom_text .= '<input type="text" name="'.esc_attr( $name.'['.$id.'][custom_tagline]' ).'" value="'.esc_attr($current_tagline).'" /></p>';
	$custom_text .= '<p class="note">'.__( 'Insert your custom text.', WS_GETTEXT_DOMAIN ).'</p>';
	...
	$image_upload .= '<p class="note">'.__( 'Use the "Upload" button to either upload an image or select an image from your media library. You can also type in the URL to an image in the text field.', WS_GETTEXT_DOMAIN ).'</p>';
...
function wordstrap_social_media_option( $id, $name, $val ) {
	...
	$sources = apply_filters( 'wordstrap_social_media_buttons', $sources );
...
function wordstrap_sidebar_layout_dropdown( $layout = null ) {
	$sidebar_layouts = wordstrap_sidebar_layouts();
	$output = '<p><strong>'.__( 'Sidebar Layout', WS_GETTEXT_DOMAIN ).'</strong></p>';
	$output .= '<select name="_ws_sidebar_layout">';
	$output .= '<option value="default">'.__( 'Default Sidebar Layout', WS_GETTEXT_DOMAIN ).'</option>';
...
function wordstrap_custom_layout_dropdown( $layout = null ) {
	$custom_layouts = get_posts('post_type=ws_layout&numberposts=-1');
	$output = '<p><strong>'.__( 'Custom Layout', WS_GETTEXT_DOMAIN ).'</strong></p>';
	if( ! empty( $custom_layouts ) ) {
		$output .= '<select name="_ws_custom_layout">';
		...
		$output .='<p class="ws_custom_layout"><em>'.__( 'You haven\'t created any custom layouts in the Layout builder yet.', WS_GETTEXT_DOMAIN ).'</em></p>';
	}
	return $output;
}
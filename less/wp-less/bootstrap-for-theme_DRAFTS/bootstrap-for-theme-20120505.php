...
if (!class_exists('WPLessPlugin'))
{
  ...
  $active_theme_uri = get_template_directory_uri();
  $active_theme_dir = get_template_directory();
  $WPLessPlugin->getConfiguration()->setUploadUrl($active_theme_uri.'/less/');
  $WPLessPlugin->getConfiguration()->setUploadDir($active_theme_dir.'/less/');
  $WPLessPlugin->dispatch();
}
...
if (!is_admin())
{
	...
	if ( of_get_option('alienship_enable_custom_styles') ) {
	
	    	$bs_style_body = of_get_option('alienship_style_body');
		$WPLessPlugin->addVariable('@bodyBackground', $bs_style_body);
		
	    	$bs_style_text = of_get_option('alienship_style_text');
		...
		$WPLessPlugin->addVariable('@linkColor', $bs_style_link);
	    	...
	    	$bs_style_hr = of_get_option('alienship_style_hr');
		$WPLessPlugin->addVariable('@hrBorder', $bs_style_hr);		
		
	    	$bs_style_navbar_height = of_get_option('alienship_style_navbar_height');
	...
	else if (! of_get_option('alienship_enable_custom_styles') ) {
	...
	if ( of_get_option('alienship_responsive') ) { 	
